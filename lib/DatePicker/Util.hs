...
monthWeeks :: Month -> Cal.DayOfWeek -> NE.NonEmpty Week
monthWeeks m dw = NE.fromList $ map NE.fromList (monthWeeks' $ Cal.periodFirstDay m)
  where
    weekOfDay :: Cal.Day -> [Cal.Day]
    weekOfDay = Cal.weekAllDays dw
